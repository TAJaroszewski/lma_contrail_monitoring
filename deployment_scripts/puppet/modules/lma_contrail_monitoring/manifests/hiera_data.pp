...
notice('fuel-plugin-lma-contrail: hiera_data')

define lma_contrail_monitoring::hiera_data (
  $content,
  $ensure = present
) {
  ...
  # Hiera needs to be aware of contrail definitions yaml in /etc/hiera/override
  ...
  validate_hash($parsed_yaml['lma_collector_contrail'])
